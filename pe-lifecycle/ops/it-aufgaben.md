...
| IT-8 | **Hoch** | **PDF-Reporting-Pipeline** | Reporting-PDFs sind teils **bildbasiert** (Slides als Grafiken); reine Textextraktion liefert nichts. `poppler-utils` fehlt (blockiert die Vision-`Read`-Pipeline). **AK:** `poppler-utils` + `pymupdf` bereitstellen; Render-zu-PNG-Schritt fuer `pe-report-extractor` etablieren; fuer bildbasierte Decks OCR/Vision einplanen (reine Textextraktion reicht nicht). |
